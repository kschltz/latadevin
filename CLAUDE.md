...
This project provides a local knowledge base for Claude Code backed by Datalevin, organized using Zettelkasten principles with a 3-layer hierarchy.
...
- The `UserPromptSubmit` hook automatically searches on every prompt, but you can also search explicitly for deeper context

## Auto-Recall Hook

A `UserPromptSubmit` hook automatically searches the knowledge base for keywords in the user's prompt. Relevant entries appear as `<knowledge-base-context>` in the conversation, with parent summary context included for navigation. This is passive — you should still search explicitly when you need deeper context.

## Database Location

The knowledge base lives at `~/.claude/datalevin-kb` by default. Override with the `CLAUDE_KB_PATH` env var.
